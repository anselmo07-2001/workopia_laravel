<header class="bg-blue-900 text-white p-4" x-data="{ open: false }">
    <div class="container mx-auto flex justify-between items-center">
        ...
        <nav class="hidden md:flex items-center space-x-4">
            ...
            @auth
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ route('dashboard') }}">
                        ...
                        @else
                            <img src="{{ asset('storage/avatars/default-avatar.jpg') }}" alt="{{ Auth::user()->name }}"
                                class="w-10 h-10 rounded-full">
                        @endif
                    </a>
                </div>
            ...
            {{-- <a
                ...
                class="bg-yellow-500 hover:bg-yellow-600 text-black px-4 py-2 rounded hover:shadow-md transition duration-300 {{ request()->is("jobs/created") ? " text-yellow-500" : "" }}"
            >
    ...
    >
        ...
        @auth
            ...
            <x-logout-button />
            <div class="pt-2"></div>
            <x-button-link url="/jobs/created" :active="request()->is('jobs/created')" :mobile="true" :block="true">
                Create Job
            </x-button-link>
        @else
